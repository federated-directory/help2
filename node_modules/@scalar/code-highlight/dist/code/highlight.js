...
function syntaxHighlight(codeString, options) {
    ...
    const credentials = (typeof options?.maskCredentials === 'string'
        ? [options.maskCredentials]
        : options?.maskCredentials ?? []).filter((c) => {
        if (c.length < 3) {
            console.error(`Codeblock credentials must be at least 3 characters to mask. Will not mask "${c}"`);
            return false;
        }
        return true;
    });
    ...
    return credentials.length
        ? htmlString.replace(new RegExp(credentials.join('|'), 'g'), (m) => `<span class="credentials">${m}</span>`)
        : htmlString;
}
/**
